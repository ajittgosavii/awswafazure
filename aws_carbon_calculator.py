...
def get_carbon_data_from_aws(session: boto3.Session, regions: List[str] = None) -> Dict:
    ...
    This replaces the dummy data in Live mode
    """
    ...
    result = calculator.calculate_total_emissions(regions, days=30)
    
    if result['success']:
        return {
            'total_emissions_kg': result['total_emissions_kg'],
            'emissions_by_service': result['emissions_by_service'],
            'emissions_by_region': result['emissions_by_region'],
            'sustainability_score': result['sustainability_score'],
            'renewable_energy_pct': result['renewable_energy_percentage'],
